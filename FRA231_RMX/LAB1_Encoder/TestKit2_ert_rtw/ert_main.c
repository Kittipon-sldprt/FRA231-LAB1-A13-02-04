...
void rt_OneStep(void)
{
  ...
  __enable_irq();
  currentTime = (extmodeSimulationTime_T) TestKit2_M->Timing.taskTime0;
  TestKit2_step();
...
int main(int argc, char **argv)
{
  ...
  MX_TIM1_Init();
  MX_TIM4_Init();
  rtmSetErrorStatus(TestKit2_M, 0);
  ...
  errorCode = extmodeInit(TestKit2_M->extModeInfo, &rtmGetTFinal(TestKit2_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(TestKit2_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(TestKit2_M);
  __enable_irq();
  __enable_irq();
  while (runModel) {
